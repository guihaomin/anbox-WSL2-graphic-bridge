...
#include "anbox/platform/sdl/windowplatform.h"
...
Window::Window(const std::shared_ptr<Renderer> &renderer,
  ...
  native_window_ = (EGLNativeWindowType)(info.info.win.window);
  ...
  auto x = 0x00;
  while (true) {
      SDL_FillRect(surface, NULL, SDL_MapRGB(surface->format, 0xff, x, 0x00));
      x++;
