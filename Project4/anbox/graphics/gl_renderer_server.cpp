...
GLRendererServer::GLRendererServer(const Config &config, const std::shared_ptr<wm::Manager> &wm)
  ...
  if (config.driver == Config::Driver::Software) {
    //auto swiftshader_path = fs::path(utils::get_env_value("SWIFTSHADER_PATH"));
      auto swiftshader_path = fs::path("E:/archive/backup/translator");
    ...
    gl_libs = std::vector<emugl::GLLibrary>{
      {emugl::GLLibrary::Type::EGL, (swiftshader_path / "libEGL_translator.dll").string()},
      {emugl::GLLibrary::Type::GLESv1, (swiftshader_path / "libGLES_CM_translator.dll").string()},
      {emugl::GLLibrary::Type::GLESv2, (swiftshader_path / "libGLES_V2_translator.dll").string()},
    };
  }
