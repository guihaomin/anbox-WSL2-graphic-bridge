...
#include "external\android-emugl\host\libs\GLESv2_dec\gles2_dec.h"
...
bool Renderer::initialize(EGLNativeDisplayType nativeDisplay) {
  ...
  //const char* resultt = eglQueryString(m_eglDisplay, EGL_VERSION);
  ...
  GLint surfaceType = EGL_WINDOW_BIT;
...
void Renderer::swapWindowBuffer() {
    std::map<EGLNativeWindowType, RendererWindow*>::iterator itr;
    for (itr == m_nativeWindows.begin(); itr != m_nativeWindows.end(); itr++) {
        s_egl.eglSwapBuffers(m_eglDisplay, itr->second->surface);
    }
}
...
void Renderer::setupViewport(RendererWindow *window,
  ...
  float const near2 = (rect.height() / 2.0f) /
                     std::tan((vertical_fov_degrees * M_PI / 180.0f) / 2.0f);
  float const far2 = -near2;
