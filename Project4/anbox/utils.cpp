...
std::string process_get_exe_path(const pid_t &pid) {
    char exe_path_char[200];
    GetModuleFileNameA(NULL, exe_path_char, sizeof(exe_path_char));
...
bool is_mounted(const std::string &path) {
  ...
  return is_mounted;*/
    // container manager will not be ran on windows host function will not be called.
    return true;
