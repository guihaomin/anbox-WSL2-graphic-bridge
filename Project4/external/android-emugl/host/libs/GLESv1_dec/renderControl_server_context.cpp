...
int renderControl_server_context_t::initDispatchByName(void *(*getProc)(const char *, void *userData), void *userData)
{
	...
	rcSelectChecksumCalculator = (rcSelectChecksumCalculator_server_proc_t) getProc("rcSelectChecksumCalculator", userData);
	rcGetNumDisplays = (rcGetNumDisplays_server_proc_t) getProc("rcGetNumDisplays", userData);
	rcGetDisplayWidth = (rcGetDisplayWidth_server_proc_t) getProc("rcGetDisplayWidth", userData);
	rcGetDisplayHeight = (rcGetDisplayHeight_server_proc_t) getProc("rcGetDisplayHeight", userData);
	rcGetDisplayDpiX = (rcGetDisplayDpiX_server_proc_t) getProc("rcGetDisplayDpiX", userData);
	rcGetDisplayDpiY = (rcGetDisplayDpiY_server_proc_t) getProc("rcGetDisplayDpiY", userData);
	rcGetDisplayVsyncPeriod = (rcGetDisplayVsyncPeriod_server_proc_t) getProc("rcGetDisplayVsyncPeriod", userData);
	rcPostLayer = (rcPostLayer_server_proc_t) getProc("rcPostLayer", userData);
	rcPostAllLayersDone = (rcPostAllLayersDone_server_proc_t) getProc("rcPostAllLayersDone", userData);
	return 0;
}
