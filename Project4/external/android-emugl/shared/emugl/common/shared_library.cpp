...
#include "external\android-emugl\shared\emugl\common\shared_library.h"
...
SharedLibrary* SharedLibrary::open(const char* libraryName,
    ...
    MultiByteToWideChar(CP_ACP, 0, libraryName, -1, wString, 4096);
    HMODULE lib = LoadLibrary(wString);
    if (lib) {
